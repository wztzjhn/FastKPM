...
namespace fkpm {
    ...
    public:
        ...
        arma::SpMat<T> to_arma() const {
            ...
            locations.row(0) = arma::urowvec(row_idx);
            locations.row(1) = arma::urowvec(col_idx);
    ...
    public:
        ...
        T& operator()(int i, int j) {
            for (uword p = row_ptr[i]; p < row_ptr[i+1]; p++) {
                if (this->col_idx[p] == j) {
                    return this->val[p];
                }
            }
            std::cerr << "Could not find index (" << i << "," << j << ") in compressed CSR matrix.\n";
            abort();
        }
